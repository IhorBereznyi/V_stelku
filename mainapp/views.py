...
class Base_ViewPriceLow(CartMixin, DetailView):
...
class Base_ViewPriceH(CartMixin, DetailView):
...
class Base_ViewPrice_Sale(CartMixin, DetailView):

    def get(self, request, *args, **kwargs):
        categories = Category.objects.all()
        products = Product.objects.all().order_by('-sale')[:25]
        context = {
            'categories': categories,
            'products': products,
            'cart': self.cart
...
class BaseView_data_new(CartMixin, View):

    def get(self, request, *args, **kwargs):
        categories = Category.objects.all()
        products = Product.objects.all().order_by('-id')[:100] #show latest 20 product
        context = {
            'categories': categories,
            'products': products,
            'cart': self.cart
...
class BaseView_data_old(CartMixin, View):
...
class BaseView(CartMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        categories = Category.objects.all()
        products = Product.objects.all().order_by('-id')[:20] #show latest 20 product
        context = {
            'categories': categories,
            'products': products,
            'cart': self.cart
...
class CategoryDetailView(CartMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if not query and not self.request.GET:
            context['category_products'] = category.product_set.all().order_by('-id')
            return context
        if query:
...
class AddToCartView(CartMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        messages.add_message(request, messages.INFO, "Товар успішно добавлено")
...
class MakeOrderView(CartMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            messages.add_message(request, messages.INFO, 'Дякуємо за замовлення! Менеджер с Вами зв`яжеться')
