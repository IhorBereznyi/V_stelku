...
class CreateFeatureView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not created:
            return JsonResponse({
                "error": f"Значення '{value}' уже існує."
            })
        messages.add_message(
            request, messages.SUCCESS,
            f'Значення "{value}" для характеристики '
            f'"{feature.feature_name}" в категорії {category.name} успішно створено'
        )
...
class UpdateProductFeaturesAjaxView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        messages.add_message(
            request, messages.SUCCESS,
            f'Значення характеристики для товара {product.title} успішно обновлено'
        )
        return JsonResponse({"result": "ok"})
